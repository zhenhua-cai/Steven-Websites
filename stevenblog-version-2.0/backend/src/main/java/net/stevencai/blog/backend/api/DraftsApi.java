...
public class DraftsApi extends PostApi{
    ...
    @GetMapping
    public ArticleResourceResponse getArticleDrafts(@RequestParam("page") int page, @RequestParam("size") int size) {
        Page<ArticleDraft> pageable = draftService.findArticleDrafts(page, size);
        return constructArticleResourceResponse(pageable);
    }

    @GetMapping("/search")
    public ArticleResourceResponse searchArticleDraftsByAuthorOrTitle(
    ...
    @GetMapping("/{id}")
    public ArticleResource getArticleDraft(@PathVariable("id") String id) {
        return draftService.findArticleDraftById(id);
    }
